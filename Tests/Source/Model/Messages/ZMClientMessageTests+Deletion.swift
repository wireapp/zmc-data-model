...
class ZMClientMessageTests_Deletion: BaseZMClientMessageTests {
    ...
    func testThatItDeletesAnAssetMessage_Image() {
        // given
        let conversation = ZMConversation.insertNewObject(in:uiMOC)
        guard let sut = conversation.appendOTRMessage(withImageData: mediumJPEGData(), nonce: .create()) else { return XCTFail() }
    ...
    func testThatItDeletesAnAssetMessage_File() {
        ...
        guard let sut = conversation.appendOTRMessage(with: fileMetaData, nonce: .create()) else { return XCTFail() }
    ...
    func testThatTheInsertedDeleteMessageDoesNotHaveAnExpirationDate() {
        ...
        // when
        guard let sut = conversation.append(deletedMessage, expires: false, hidden: true) else { return XCTFail() }
