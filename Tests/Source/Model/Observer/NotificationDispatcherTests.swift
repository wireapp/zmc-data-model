...
class NotificationDispatcherTests : ZMBaseManagedObjectTest {
    ...
    func testThatItNotifiesTheObserverOfChangedUnreadCount()
    {
        ...
        XCTAssert(waitForAllGroupsToBeEmpty(withTimeout: 0.5))
        self.uiMOC.refresh(uiConversation, mergeChanges: true)
        XCTAssert(waitForAllGroupsToBeEmpty(withTimeout: 0.5))
        ...
        XCTAssert(waitForAllGroupsToBeEmpty(withTimeout: 0.5))
        let changedObjects = (mergeNotifications.last?.userInfo?[NSUpdatedObjectsKey] as? Set<ZMManagedObject>)?.map{$0.objectID} ?? []
        self.sut.willMergeChanges(changes: changedObjects)
    ...
    func testThatItNotifiesTheObserverOfAChangedListIndicatorBecauseOfAnUnreadMissedCall()
    {
        ...
        XCTAssert(waitForAllGroupsToBeEmpty(withTimeout: 0.5))
        let lastMergeInfo = mergeNotifications.last
        let changedObjects = (lastMergeInfo!.userInfo?[NSUpdatedObjectsKey] as? Set<ZMManagedObject>)?.map{$0.objectID} ?? []
        self.sut.willMergeChanges(changes: changedObjects)
        self.uiMOC.mergeChanges(fromContextDidSave: lastMergeInfo!)
//        self.uiMOC.refresh(uiConversation, mergeChanges: true)
        XCTAssert(waitForAllGroupsToBeEmpty(withTimeout: 0.5))

        self.sut.fireAllNotifications() // the context does not save here
    ...
    {
        // measured [Time, seconds] average: 0.092, relative standard deviation: 7.991%, values: [0.113412, 0.090856, 0.089037, 0.088853, 0.089265, 0.089114, 0.089649, 0.087644, 0.088816, 0.089100
